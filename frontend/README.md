...
The frontend is hosted on Vercel, accessible [here](https://queue-care-psi.vercel.app/). However, the application **cannot be used through this link unless the backend is running locally on your machine first**.
...
To avoid flaky tests and ensure stable and consistent selectors, every interactive element on the app carries a `data-cy` attribute generated through `src/utils/cy.js`. Cypress specs target these attributes exclusively.
